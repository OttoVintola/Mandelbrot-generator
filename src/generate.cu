...
int main() {
    int width, height = 0; // These control the ???
    ...
    dim3 blockSize(16, 16); // Nor this
    dim3 gridSize(...);

    generate<<gridSize, blockSize, nSRAMBytesPerBlock, stream>>>(...); 
}